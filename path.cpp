...
void ApplyRingDamage(Player &player, float innerRadius, float outerRadius, float deltaTime)
{
    ...
    // Too match the center of the red ring
    Vector2 ringCenter = {400.0f, 300.0f};
// too get the horizontal and vertical distance from the player to the center of the ring
    float dx = player.pos.x - ringCenter.x;
    float dy = player.pos.y - ringCenter.y;
    float distance = sqrtf(dx * dx + dy * dy); // Caculates the distance from the player to the center of the ring

    // Touching the red band?
    bool touchingRing =
...
int main() {
    ...
    while (!WindowShouldClose()) { // runs until the window is closed (e.g. Esc or X button)
        ...
        ApplyRingDamage(p1, innerRadius, outerRadius, delta); // apply damage to player 1 if touching the red ring
        ApplyRingDamage(p2, innerRadius, outerRadius, delta); // apply damage to player 2 if touching the red ring
